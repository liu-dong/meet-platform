...
public class JDBCUtils {
     ...
     * 数据库连接地址
     */
    public static final String URL = "jdbc:mysql://192.168.154.127:3306?useSSL=false&characterEncoding=utf-8";
    /**
     * 数据库的用户名
     */
...
//            conn = DriverManager.getConnection(URL, USER, PASSWORD);
     ...
     * 获得数据库中所有Catalogs
     */
    public static void getCatalogList() {
        System.out.println("=========================获得数据库中所有Catalogs=========================");
        Connection conn = getConnection();
        ResultSet rs = null;
        try {
            DatabaseMetaData metaData = conn.getMetaData();
    ...
    public static void getTablesList() {
        System.out.println("=========================获取数据库中所有的表信息=========================");
        Connection conn = getConnection();
        ResultSet rs = null;
        try {
            DatabaseMetaData metaData = conn.getMetaData();
            String[] types = {"TABLE","VIEW"};
            rs = metaData.getTables("my_data", null, "%", types);
            while (rs.next()) {
