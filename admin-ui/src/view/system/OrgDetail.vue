    ...
    <div class="bottom">
      <el-form ref="ruleForm" :model="ruleForm" :rules="rules" class="form" label-width="100px">
        ...
        <el-form-item label="单位地址" prop="orgDivisionCode" style="width: 70%;">
          <el-cascader :key="modalKey" ref="cascaderDivision" v-model="divisionCodes"
                       :props=divisionProps separator="" style="width: 100%" @change="setOrgAddress"></el-cascader>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    setOrgAddress(value) {
      ...
      let pathLabels = this.$refs["cascaderDivision"].getCheckedNodes()[0].pathLabels;
    ...
    lazyLoadDivision(node, resolve) {
      ...
      if (level === 0) {
        findProvinceList({}).then(res => {
          if (res.code === 200) {
            let divisionList = res.data
            let nodes = divisionList.map(item => ({
