...
public class QRCodeUtils {
    ...
    /**
     * @param codeContent 二维码参数内容，
     *                    如果是一个网页地址，如 https://www.baidu.com/ 则 微信扫一扫会直接进入此地址，
     *                    如果是一些参数，如1541656080837，则微信扫一扫会直接回显这些参数值
     * @param saveDir     二维码图片保存的目录,如 D:/codes
     * @param fileName    二维码图片文件名称，带格式,如 123.png
     */
    public static void createCodeToFile(String codeContent, File saveDir, String fileName) {
        try {
            ...
            if (saveDir == null || saveDir.isFile()) {
                saveDir = FileSystemView.getFileSystemView().getHomeDirectory();
             ...
             */
            File codeImg = new File(saveDir, fileName);
            ImageIO.write(bufferedImage, "png", codeImg);
            System.out.println("二维码图片生成成功：" + codeImg.getPath());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static String parseQRCodeByFile(File file) {
        ...
        try {
            ...
            path = parseQRCode(bufferedImage);
    ...
    public static String parseQRCodeByUrl(URL url) {
        ...
        try {
            ...
            e.printStackTrace();
            System.err.println("二维码图片地址错误, 地址是: " + url);
        }
        return path;
    }
    ...
    public static void main(String[] args) {
        QRCodeUtils.createCodeToFile("不得语，暗相思。两心之外无人知。——致妖姬", null, null);
    }
