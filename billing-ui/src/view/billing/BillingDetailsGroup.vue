...
<script>
import {findBillingListGroup} from "@/api/billingDetails";
...
export default {
  ...
  methods: {
    findBillingListGroup(){
      let data = {}
      findBillingListGroup(data).then(res => {
        if (res.code === 200){
          this.billingDetailsGroup = res.data
          console.log('billingDetailsGroup：',this.billingDetailsGroup)
          console.log('billingDetailsGroup1：',this.billingDetailsGroup['2022-05-30'])
        }
      })
    }
  }
